REPO       := github.com/xdu31/exercise-service
...
PSQL                    := docker exec -i exercise-service-db psql -U test_user -d test -v ON_ERROR_STOP=1
...
.PHONY: image-test-db
image-test-db:
	@docker build -f $(CUR_DIR)/db/Dockerfile.tests -t exercise-service-db:latest .
